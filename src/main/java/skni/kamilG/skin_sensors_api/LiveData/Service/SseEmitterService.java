...
public class SseEmitterService implements ISseEmitterService {
    ...
    private void sendInitialState(String clientId, Short sensorId) {
        try {
            ...
            if (emitter != null) {
                try {
                    ...
                    log.debug("Failed to send initial state to client {}: {}", clientId, e.getMessage());
                    removeEmitter(clientId);
                }
            }
        } catch (Exception e) {
            log.error("Error sending initial state for sensor {}: {}", sensorId, e.getMessage());
        }
    }

    private void sendInitialStatesForAllSensors(String clientId) {
        ...
        for (SensorResponse sensor : sensors) {
            try {
                ...
                emitter.send(event);
                log.debug("Sent initial state for sensor {} to client {}", sensor.id(), clientId);

            } catch (IOException e) {
                log.warn("Failed to send initial state of sensor {} to client {}: {}",
                        sensor.id(), clientId, e.getMessage());
                removeEmitter(clientId);
                break;
            } catch (Exception e) {
                log.error("Error building event for sensor {}: {}", sensor.id(), e.getMessage());
            }
        }
        log.debug("Sent all initial states to client: {}", clientId);
    }
    ...
    private void subscribeClientToSensors(String clientId, Short sensorId) {
        if (sensorId != null) {
            ...
            taskScheduler.schedule(() -> sendInitialState(clientId, sensorId), Instant.now());
        } else {
            allSensorsSubscribers.add(clientId);
            taskScheduler.schedule(() -> sendInitialStatesForAllSensors(clientId), Instant.now());
        }
